...
import { setLoading, setProducts } from "./ProductSlice";
import SwiperComponent, { SwiperComponentCustom } from "../../Component/Swiper/Swiper";
...
function Product(props) {
  ...
  useEffect(() => {
    if(productState.query && productState.query != ""){
      fetchProducts(
        productState.query,
        productState.pageIndex,
        productState.pageSize
      );
    }else if(productState.categoryId && productState.categoryId != ""){
  ...
  }, [productState.query, productState.pageIndex, productState.pageSize , productState.categoryId , productState.brandId]);
  ...
  const fetchProductsByBrand = async (brandId) => {
    ...
    try {
      const response = await getRequests(
        `${base_url}/api/v1/brand/${brandId}`
      ); 
      let { productList} = response.data;
      dispatch(setProducts({ products: productList, length: productList?.length }));
    } catch (error) {
      console.error("Error fetching products", error);
      dispatch(setProducts({ products: [], length: 0 }));
    } finally {
